...
if [ -z "$NEOVIM_PATH" ]; then
    ...
    zshrc_path="$USER_HOME/.bashrc"
    echo "# Neovim" >> "$zshrc_path"
    echo "export NEOVIM_PATH=\"$installation_dir/$asset_name/bin/nvim\"" >> "$zshrc_path"
    echo 'alias vim="$NEOVIM_PATH"' >> "$zshrc_path"
    echo 'export SUDO_EDITOR="$NEOVIM_PATH"' >> "$zshrc_path"
    echo "" >> "$zshrc_path"
    echo "$zshrc_path has been updated"
fi
